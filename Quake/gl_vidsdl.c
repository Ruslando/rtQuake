...
static void GL_CreateColorBuffer(void)
{
	...
	for (i = 0; i < NUM_COLOR_BUFFERS; ++i)
	{
		...
		GL_SetObjectName((uint64_t)color_buffers_memory[i], VK_OBJECT_TYPE_DEVICE_MEMORY, va("Color Buffer Memory %d", i));
...
qboolean GL_BeginRendering(int* x, int* y, int* width, int* height)
{
	...
	*height = vid.height;
	...
	// Raytrace Render Pass Begin Infos
	{
