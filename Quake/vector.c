...
int vectorPushBackValue(vector* v, const void* item, size_t size)
{
    ...
    if (v)
    {
        if (v->vectorList.capacity == v->vectorList.total)
        {
            ...
            if (status != UNDEFINE)
            {
                v->vectorList.items[v->vectorList.total++] = copy;
            }
        }
        else
        {
...
void vector_init(vector* v)
{
    ...
    v->pfVectorPushBackReference = vectorPushBackReference;
    v->pfVectorPushBackValue = vectorPushBackValue;
